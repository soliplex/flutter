...
/// Theme extension for markdown rendering styles.
///
/// Register this in the app's [ThemeData.extensions] to drive markdown
/// appearance from the theme system. The concrete renderer reads it via
/// `Theme.of(context).extension<MarkdownThemeExtension>()`.
class MarkdownThemeExtension extends ThemeExtension<MarkdownThemeExtension> {
  ...
  /// Converts to the package-specific [MarkdownStyleSheet].
  ///
  /// When [codeFontStyle] is provided, it is merged with [code] so that
  /// platform-specific font selection (determined at build time) combines
  /// with the theme's chromatic properties (background color, etc.).
