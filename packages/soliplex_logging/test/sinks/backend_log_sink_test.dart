  ...
  group('BackendLogSink', () {
    ...
    test(
      ...
      () async {
        ...
        // Three consecutive failures (clear backoff between each).
        await sink.flush();
    ...
    test('re-enables after new JWT on 401', () async {
      ...
      // Disabled now. Simulate re-login with new JWT.
      jwt = 'new-jwt';
      httpStatus = 200;
      sink.write(makeRecord(message: 'Second'));
      await sink.flush();
    ...
    test('network error triggers retry with backoff', () async {
      ...
      // Record stays in queue after network error.
      expect(await diskQueue.pendingCount, 1);
      await sink.close();
    });
    ...
    test('record size guard truncates oversized messages', () async {
      // Create a record with a very large message (>64KB).
      ...
      // Truncated message should be much shorter than original.
    ...
    test('UTF-8 safe truncation does not split multi-byte chars', () async {
      // Create message with multi-byte characters followed by padding.
      ...
      expect(capturedRequests, hasLength(1));
      // If truncation was not UTF-8 safe, jsonDecode would fail.
      final body =
          jsonDecode(capturedRequests.first.body) as Map<String, Object?>;
