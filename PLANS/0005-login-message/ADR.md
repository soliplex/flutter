...
Some deployments require a notice-and-consent banner before system access
([#751](https://github.com/enfold/afsoc-rag/issues/751),
[#815](https://github.com/enfold/afsoc-rag/issues/815)). This must be
configurable — most deployments should not see it.
...
**Why `body` is a plain String:** The body is rendered in a scrollable view
to accommodate long text. No markdown or rich text needed — consent banners
are typically plain prose. Shell apps pass the exact required text.
...
The banner must appear before deep links and after session timeout. Both are
already handled by the router's auth guard:
...
- No persistence needed (re-acknowledgment required every session).
